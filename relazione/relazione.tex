...
All'interno dei vari file abbiamo utilizzato sempre link relativi, salvo alcune occasioni in cui ci è tornato comodo scrivere link assoluti in maniera dinamica (tramite php) e il file \texttt{.htaccess}, che invece richiede l'utilizzo di percorsi assoluti.
...
\begin{itemize}
	...
	\item Il codice HTML è sintatticamente corretto
	
	\item Abbiamo cercato di rendere le pagine leggere mettendo le immagini con dimensioni più vicine possibile a quanto effettivamente utilizzate, favorendo i file \texttt{JPEG} e utilizzando immagini di grandezza diversa quando la grandezza attesa è diversa (ad es. come compare la foto profilo di un istruttore nella pagina di ricerca degli istruttori, con risultati  multipli, e come compare nel suo profilo pubblico, ove il risultato è singolo)
\end{itemize}
...
\section{Utenti presenti nel sito} All'interno del sito troviamo i seguenti tipi di utenti: \begin{itemize} \item \textbf{Cliente} \item \textbf{Personal Trainer} \item \textbf{Amministratore} \end{itemize} Tutti gli utenti accedono tramite un'area riservata che mantiene una coerenza grafica (layout "Dashboard"), ma che differisce nei contenuti e negli strumenti messi a disposizione in base al ruolo. Inoltre, i Personal Trainer dispongono di un \textbf{profilo pubblico} visibile a tutti nella sezione \texttt{cerca-istruttore.php}.

Di seguito sono riportate le funzionalità specifiche:

\subsection{Cliente} \begin{itemize} \item Visualizzazione dello stato dell'abbonamento e dei dati anagrafici. \item Download della scheda di allenamento (PDF) caricata dal proprio trainer. \item Accesso rapido al profilo del proprio Personal Trainer e al calendario corsi. \item Gestione del proprio account (modifica e-mail, password, telefono). \end{itemize}

\subsection{Personal Trainer} \begin{itemize} \item Accesso alla lista dei Clienti assegnati. \item Funzionalità di upload per caricare le schede tecniche (PDF) associate ai clienti. \item Gestione del proprio account (modifica e-mail, password, telefono) e visualizzazione calendario corsi. \item (Il profilo pubblico viene generato automaticamente dai dati inseriti dall'amministratore). \end{itemize}

\subsection{Amministratore} \begin{itemize} \item Gestione completa (visualizzazione, modifica ed eliminazione) delle liste: Clienti, Personal Trainer e Utenti Totali. \item Per i Personal Trainer: gestione di campi specifici come \textit{specializzazione} e \textit{qualifica}, che verranno mostrati nel lato pubblico. \item Modifica delle credenziali di accesso amministrativo. \end{itemize}
...
\begin{itemize}
	...
	\item In fase di stampa, il colore principale della palestra, il giallo, viene trasformato in nero per favorire la lettura.
\end{itemize}
...
In merito a questo, nella pagina \texttt{Home} è presente un \texttt{lang="fr"} per \textit{tapis roulant} che, per qualche motivo, NVDA non legge in francese, nonostante notifichi il cambio di lingua.

\subsection{Screen reader}
La separazione tra struttura, presentazione e comportamento garantisce che il sito sia accessibile a tutti gli utenti, inclusi quelli che utilizzano screen reader o dispositivi assistive. L'HTML semantico facilita la navigazione e la comprensione del contenuto.
...
Abbiamo adottato un menù ad hamburger con css puro per gli utenti da mobile, facilitandone la navigazione; le varie tabelle presenti inoltre sono state linearizzate in modo fa facilitarne la lettura su diversi dispositivi.
...
\begin{itemize}
    ...
    \item Crezione della prima versione del database \texttt{db.sql} e mantenimento dello stesso
	\item Creazione e gestione dei file:
	\begin{itemize}
		\item \texttt{login.php} (con css mobile, dekstop e stampa)
		\item \texttt{registrati.php} (con css mobile, dekstop e stampa)
		\item \texttt{contattaci.php} (con css mobile, dekstop e stampa)
