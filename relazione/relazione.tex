...
Per agevolare l’orientamento dell’utente all’interno della gerarchia del sito, ogni pagina interna presenta un percorso di navigazione (breadcrumb). Tale elemento è racchiuso in un contenitore semantico \texttt{<nav>} e identificato tramite l’attributo \texttt{aria-label="percorso di navigazione"}, rendendolo distinguibile dal menu principale per gli screen reader. L’ultimo elemento del percorso, corrispondente alla pagina corrente, è contrassegnato con l’attributo \texttt{aria-current="page"}, permettendo all’utente di riconoscere immediatamente la propria posizione nel sito.
